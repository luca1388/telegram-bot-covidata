...
const commandsRouter = {
  "/italia": async (message, res) => {
    ...
    let responseText = `<strong>Casi totali: </strong> ${data.cases}<br/><strong>Deceduti totali: </strong> ${data.deaths}<br/><strong>Malati attuali: </strong> ${data.active}<br/><strong>Casi di oggi: </strong> ${data.todayCases}<br/><strong>Deceduti di oggi: </strong> ${data.todayDeaths}`;
