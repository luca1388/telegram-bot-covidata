...
const commandsRouter = {
  "/italia": async (message, res) => {
    ...
    let responseText = `<strong>Casi totali: </strong> ${data.cases} <strong>Deceduti totali: </strong> ${data.deaths} <strong>Malati attuali: </strong> ${data.active} <strong>Casi di oggi: </strong> ${data.todayCases} <strong>Deceduti di oggi: </strong> ${data.todayDeaths}`;
